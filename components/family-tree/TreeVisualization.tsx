...
export const TreeVisualization: React.FC<TreeVisualizationProps> = ({ persons, relationships, onPersonClick }) => {
  ...
  const getGender = (person: Person) => {
    const genderMap: Record<string, string> = { male: '👨', female: '👩', other: '🧑', not_specified: '👤' };
      ...
      <div className="inline-block min-w-full p-8 bg-white dark:bg-gray-800 rounded-lg">
        {rootPersons.length === 0 ? (
          <div className="flex items-center justify-center h-96 text-center">
            <div>
              <p className="text-gray-500 dark:text-gray-400 text-lg">No family tree structure</p>
              <p className="text-gray-400 dark:text-gray-600 text-sm mt-2">Add members and relationships</p>
            </div>
          </div>
        ) : (
          <div className="space-y-12">
            {rootPersons.map((root) => (
              <TreeBranch key={root.person_id} person={root} children={parentChildMap.get(root.person_id) || []} personMap={personMap} parentChildMap={parentChildMap} onPersonClick={onPersonClick} getGender={getGender} />
            ))}
          </div>
        )}
      </div>
...
const TreeBranch: React.FC<TreeBranchProps> = ({ person, children, personMap, parentChildMap, onPersonClick, getGender }) => {
  ...
  return (
    <div className="flex flex-col items-start">
      <div onClick={() => onPersonClick?.(person)} className="group cursor-pointer mb-6 transition-transform hover:scale-105">
        <div className="relative bg-gradient-to-br p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow border-2" style={{ borderColor: textColor, backgroundColor: `${textColor}10` }}>
        ...
        <div className="relative ml-8 pl-4 border-l-2 border-gray-300 dark:border-gray-600">
          ...
          <div className="space-y-8">
            {childPersons.map((child) => (
              <TreeBranch key={child.person_id} person={child} children={parentChildMap.get(child.person_id) || []} personMap={personMap} parentChildMap={parentChildMap} onPersonClick={onPersonClick} getGender={getGender} />
            ))}
          </div>
        </div>
      )}
